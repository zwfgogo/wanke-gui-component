...
class mxConnectionHandler extends mxEventSource {
   ...
   */
  createMarker() {
    return new CellMarker1(this, this.graph)
  }
...
export class CellMarker1 extends mxCellMarker {
  connectionHandler = null

  constructor(connectionHandler, graph, validColor, invalidColor, hotspot) {
  ...
  getCell(me) {
    ...
    if (cell == null && this.connectionHandler.currentPoint != null) {
      cell = this.graph.getCellAt(connectionHandler.currentPoint.x, connectionHandler.currentPoint.y)
    }
    ...
    if ((this.graph.isSwimlane(cell) && connectionHandler.currentPoint != null &&
      this.graph.hitsSwimlaneContent(cell, connectionHandler.currentPoint.x, connectionHandler.currentPoint.y)) ||
    ...
    if (cell != null) {
      if (connectionHandler.isConnecting()) {
        if (connectionHandler.previous != null) {
          connectionHandler.error = connectionHandler.validateConnection(this.connectionHandler.previous.cell, cell)

          if (connectionHandler.error != null && connectionHandler.error.length == 0) {
            cell = null

            // Enables create target inside groups
            if (connectionHandler.isCreateTarget(me.getEvent())) {
              connectionHandler.error = null
            }
          }
        }
      } else if (!connectionHandler.isValidSource(cell, me)) {
        cell = null
      }
    } else if (connectionHandler.isConnecting() && !connectionHandler.isCreateTarget(me.getEvent()) &&
  ...
  isValidState(state) {
    let connectionHandler = this.connectionHandler
    if (connectionHandler.isConnecting()) {
      return connectionHandler.error == null
    } else {
  ...
  getMarkerColor(evt, state, isValid) {
    let connectionHandler = this.connectionHandler
    return (connectionHandler.connectImage == null || connectionHandler.isConnecting()) ?
  ...
  intersects(state, evt) {
    let connectionHandler = this.connectionHandler
    if (connectionHandler.connectImage != null || connectionHandler.isConnecting()) {
      return true
    }
