  ...
  renderCategories() {
    ...
    return Object.keys(categories)
      .map((key: CategoriesKeys) => {
        ...
        if (key == 'wanke' || key == 'gf' || key == 'fz') {
          let startStr = key.replace(/^\w/, l => l.toUpperCase())
          return {
            category: key,
            icons: Object.keys(allIcons).filter(item=> item.indexOf(startStr) == 0).filter(item => item.indexOf(theme) != -1)
          };
        }
